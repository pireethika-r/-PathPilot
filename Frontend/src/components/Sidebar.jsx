...
export function Sidebar({ userRole, user, onLogout, currentView, setCurrentView, isOpen, setIsOpen }) {
    const navByRole = {
        student: [
            {
                id: 'profile',
    ...
    const roleLabel = userRole === 'admin' ? 'Admin' : userRole === 'hiring' ? 'Hiring Manager' : 'Student';
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-t border-slate-800">
            <div className="flex items-center px-4 py-3 rounded-lg bg-slate-800/50">
              {avatarSrc ?
            <img src={avatarSrc} alt="Profile avatar" className="w-8 h-8 rounded-full object-cover border border-slate-600 mr-3"/> :
            ...
            <button onClick={onLogout} className="mt-3 w-full px-4 py-2.5 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors">
              Log Out
            </button>
          </div>
        </div>
